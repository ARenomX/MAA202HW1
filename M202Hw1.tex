...
\end{align*}
We now consider the case where $F \subsetneq E$. The set $\{ d(y,F) \, | \, y \in F, \, N(y) = 1 \}$ can be re-written as $\{ d(x-u_x,F) \, | \, x \in F, \, d(x,F) = 1 \}$. We know that $x$ and $u_x$ are colinear, and so $d(x-u_x,F) = d(x,F) - N(u_x) = N(x) - 2N(u_x) = 1 - N(u_x)$ We are therfore looking for $\sup_{d(x,F) = 1} 1 - N(u_x)$. We know that the norm is strictly non-negative, and that $0 \in F$, so we can say that $\sup_{d(x,F) = 1} 1 - N(u_x) = 1$. This concludes the proof.
