...
%In order to show that $d(x,F)$ is attained at some $u_x \in F$, we need to show that the set $\{N(x-u) \,| \, u \in F, \, N(u) \leq 2N(x) \}$ is closed and bounded, because alongside the fact that $d$ is a continuous function and that $F$ is of finite dimension, this is sufficient to show that the infimum is contained in the set.

We want to show that $\exists \, u_x \in F, d(x,u_x) = d(x,F)$. We therefore define a function
	\begin{align*}
	f: F &\to \mathbb{R}_+\\
	u &\mapsto N(x-u) = d(x,u)
	\end{align*}
We will also define a set $U := \{u \in F \, | \, N(u) \leq 2N(x)\}$.\\ 
\noindent We have from previous questions that $d(x,F) = \inf\{ N(x-u) | u \in U\} = \inf\{ f(u) | u \in U\}$. But if $f$ is continuous and $U$ is compact then we know that the infimum is attained at some value in $U\!$. \\
	We have that $U$ is bounded. Let $M = 2N(x)$ by definition of $U, \forall u\in U, N(x) \leq M$\\
	We also have that $U$ is closed. Assume by contradiction that there exists $(x_n)_n \in U^{\mathbb{N}}$ such that it converges to $l \not \in U$. Fix $\varepsilon > 0$, there exists $n \in \mathbb{N}$ such that $N(l-x_n) <\varepsilon$
	\begin{equation*}
	N(l) = N(l-x_n+x_n) \leq N(x_n -l) + N(x_n) < \varepsilon + N(x_n) \leq  \varepsilon + 2N(x)
	\end{equation*}
	Letting $\varepsilon$ go to zero we find that $l \in U$ thus $U$ is closed. Since $U\subset F$ a finite dimensional space we have that it is compact.\\
	It now remains to prove that $f$ is continuous. Let $y, z \in F$. 
	\begin{align*}
		N(y-x) = N(y-z+z-x) \leq N(y-z) + N(z-x) \Leftrightarrow f(y) - f(z) \leq N(y-z)\\
		N(z-x) = N(z-y+y-x) \leq N(y-z) + N(y-x) \Leftrightarrow -(f(y) - f(z)) \leq N(y-z)
	\end{align*}
	Thus we have that $|f(y) - f(z)| \leq N(y-z)|$ thus $f$ is continuous.\\
	This finally gives the result that the infimum is attained for some $u_x\in F$.

\subsection{} %Exercise 2.5
The set $\{y | y = x - u_x, x \in E\}$ is effectively a scaling 
